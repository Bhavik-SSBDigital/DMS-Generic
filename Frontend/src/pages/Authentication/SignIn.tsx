  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        localStorage.setItem('specialUser', res.data['specialUser']);
        navigate('/');
      }
    } catch (error) {
      ...
      if (error.response) {
        if (error.response.status === 400) {
          ...
          setError('Client Error: ' + error.response.data.message);
        } else {
          setError('An unexpected error occurred.');
        }
      } else if (error.request) {
        setError('Network Error: Unable to make the request.');
      } else {
        setError('An unexpected error occurred.');
      }
    }
